...
function helper($time) {
    $query ="SELECT name, frequency FROM names JOIN states WHERE sex = 'M' AND year = $time GROUP BY frequency DESC LIMIT 20"; 
    return $query;
}
...
//    print_r($data);
    ...
    print "<form method=GET action='boys.php' class='text-center'><select name='time'>";
    for($i=2000;$i<2019;$i++) {
...
<script>
    window.onload = function() {
        var chart = new CanvasJS.Chart("chartContainer", {
            ...
            theme: "light2",
            title:{
                text: "<?php echo " most popular boys' name in $time"; ?>"
            },
            axisX: {
                title: "Name"
            },
